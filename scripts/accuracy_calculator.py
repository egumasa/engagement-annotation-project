...
with open("/Users/masakieguchi/Dropbox/0_Projects/0_basenlp/SFLAnalyzer/engagement-annotation-project/data/input_for_reliabiliy/11.json", 'r') as f:
	temp = json.load(f)
...
def fix_sub_type_tags(data):
	for idx, mainverb in data.items():
		...
		for funct, role in zip(theme_info['functions'], theme_info['roles']):
			if theme_info['sub_type'] in ['Equative', 'Predicated', 'Thematized comment']:
				continue
			elif funct in ["Topical", 'Interpersonal/Topical']:
				if role in ["nsubjpass", 'nsubj', 'csubj', 'csubjpass', 'Equative']:
					sub_type = "Subject"
					
				elif role in ['ccomp', 'attr', 'parataxis', 'dobj', 'acomp', 'dative']:
					sub_type = 'Complement'
...
with open(test_dir + "1_Theme_text_testset_20210715.txt", "w") as outf:
	...
	for idx, content in text_comp.items():
		tsv_writer.writerow([str(idx), str(content['tag1']), str(content['tag2']), str(content['sent'])])
...
tagged_files = glob.glob("1_corpora/1_reference/_extracted_brown//*.txt") #get list of files
...
train_data = random.sample(full_data,688) #create training set with 67% of sentences
